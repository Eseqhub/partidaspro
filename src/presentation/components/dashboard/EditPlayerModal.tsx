...
import { faStar, faSave, faArrowsUpDown, faWeightHanging, faTrash } from '@fortawesome/free-solid-svg-icons';
...
export const EditPlayerModal: React.FC<EditPlayerModalProps> = ({ 
  ...
  useEffect(() => {
    if (player && isOpen) {
      ...
      setIsMensalista(player.is_mensalista !== undefined ? player.is_mensalista : true);
      setHeight(player.height ? player.height.toString() : '');
      setWeight(player.weight ? player.weight.toString() : '');
      setPreferredFoot(player.preferred_foot || 'R');
    }
  }, [player, isOpen]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="EDITAR ATLETA">
      <div className="space-y-6 pb-6 max-h-[75vh] overflow-y-auto pr-2 no-scrollbar">
        <PhotoUploader 
           onPhotoSelected={setPhotoUrl} 
           defaultImage={photoUrl || undefined} // Permite passar a imagem existente se seu PhotoUploader aceitar
        />
        ...
        <div className="flex flex-col gap-3 mt-4">
            ...
            >
                <FontAwesomeIcon icon={faSave} /> ATUALIZAR CADASTRO
            </Button>

            {onDelete && (
                <Button 
                onClick={() => {
                    if (window.confirm("Deseja realmente arquivar este atleta?")) {
                        onDelete(player.id);
                        onClose();
                    }
                }}
                className="w-full py-4 font-black uppercase tracking-[0.3em] text-[10px] bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all gap-3 border-none"
                >
                <FontAwesomeIcon icon={faTrash} /> REMOVER ATLETA (SOFT DELETE)
                </Button>
            )}
        </div>
      </div>
