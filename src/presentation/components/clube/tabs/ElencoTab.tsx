...
import { faSearch, faChartSimple, faPen, faUserPlus, faEye } from '@fortawesome/free-solid-svg-icons';
import { PlayerEditModal } from '@/presentation/components/clube/components/PlayerEditModal';
...
export const ElencoTab: React.FC<Props> = ({ players, groupId, onNavigate, onRefresh, isOwnerOrEditor = false }) => {
  ...
  // Sincroniza quando players externos mudam
  ...
  const handleSave = (updated: Player) => {
    setLocalPlayers(prev => prev.map(p => p.id === updated.id ? updated : p));
    setEditingPlayer(null);
  };

  const handleDelete = (id: string) => {
    setLocalPlayers(prev => prev.map(p => p.id === id ? { ...p, status: 'Inativo' } : p));
  };
  ...
  return (
    <>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ padding: '14px 18px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 9, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.3)', marginRight: 4 }}>
            POR POSIÇÃO:
          </span>
          {Object.entries(byPosition).map(([pos, pl]) => (
            <div key={pos} onClick={() => setFilter('todos')}
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
            <FontAwesomeIcon icon={faChartSimple} style={{ color: gold, fontSize: 10 }} />
            <span style={{ fontSize: 10, fontWeight: 900, color: gold }}>
              {localPlayers.length > 0
                ? (localPlayers.reduce((a, p) => a + (p.skill_level ?? p.rating * 2), 0) / localPlayers.length).toFixed(1)
                : '—'}/10 média
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', flex: 1, minWidth: 160 }}>
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              ...
              style={{ width: '100%', padding: '10px 12px 10px 34px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontSize: 11, fontWeight: 700, outline: 'none', boxSizing: 'border-box' }} />
          </div>

          {/* Filtros status */}
          {(['todos', 'Ativo', 'Inativo', 'mensalista'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '8px 12px', fontSize: 9, fontWeight: 900, textTransform: 'uppercase',
          ...
          <select value={sort} onChange={e => setSort(e.target.value as any)}
            style={{ padding: '8px 12px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)',
              color: 'rgba(255,255,255,0.6)', fontSize: 9, fontWeight: 900, outline: 'none', cursor: 'pointer' }}>
        ...
        {filtered.length === 0
          ...
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 12 }}>
              {filtered.map(p => {
                ...
                const skillClr = skill >= 8 ? gold : skill >= 5 ? blue : 'rgba(255,255,255,0.3)';

                return (
                  <div key={p.id} style={{ position: 'relative', padding: 16,
                    background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)',
                    borderLeft: `3px solid ${posColor}40`, transition: 'all .2s' }}
                    ...
                    <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 6 }}>
                      {isOwnerOrEditor && (
                        <button
                          ...
                          style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center',
                            ...
                            color: blue, cursor: 'pointer', fontSize: 11, transition: 'all .2s' }}
                      ...
                      <button
                        ...
                        style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center',
                          ...
                          color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 11, transition: 'all .2s' }}
                    ...
                    {/* Info principal */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, paddingRight: isOwnerOrEditor ? 68 : 36 }}>
                      {/* Avatar */}
                      <div style={{ width: 46, height: 46, flexShrink: 0, overflow: 'hidden',
                        border: `2px solid ${posColor}33`, background: 'rgba(0,0,0,0.5)', cursor: 'pointer' }}
                        onClick={() => setEditingPlayer(p)}>
                        {p.photo_url
                          ? <img src={p.photo_url} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center',
                              justifyContent: 'center', fontSize: 18, fontWeight: 900, color: posColor }}>{p.name[0]}</div>
                        }
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 7, fontWeight: 900, padding: '2px 6px',
                            background: `${posColor}15`, color: posColor }}>{pos}</span>
                          {p.is_mensalista && (
                            <span style={{ fontSize: 7, fontWeight: 900, padding: '2px 6px',
                              background: `${gold}12`, color: gold }}>MENSAL</span>
                          )}
                          <span style={{ fontSize: 7, fontWeight: 900,
                            color: p.status === 'Ativo' ? green : 'rgba(255,255,255,0.2)' }}>
                            {p.status}
                          </span>
                        </div>
                      </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div style={{ flex: 1, height: 4, background: 'rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
                        <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', width: `${skillPct}%`,
                          background: skillClr, transition: 'width .5s' }} />
                      </div>
                      <span style={{ fontSize: 10, fontWeight: 900, color: skillClr, flexShrink: 0 }}>{skill}/10</span>
                    </div>

                    {/* Info física */}
                    {(p.height || p.weight || p.birth_date) && (
                      <div style={{ display: 'flex', gap: 10, marginTop: 8, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.04)', flexWrap: 'wrap' }}>
