  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden', padding: '20px 20px 18px',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 8, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.25)' }}>{label}</span>
      </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        <div style={{ width: 28, height: 28, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: pos ? `${green}12` : `${red}10` }}>
          <FontAwesomeIcon icon={pos ? faArrowTrendUp : faArrowTrendDown} style={{ fontSize: 10, color: pos ? green : red }} />
        </div>
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace' }}>{new Date(date).toLocaleDateString('pt-BR')}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: 20 }}>
        ...
        <div style={{ padding: 24, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {recentFinances.length === 0
            ...
            : recentFinances.map((f: any) => (
                <FinanceRow key={f.id} desc={f.description ?? f.category ?? 'Movimentação'}
                  amount={f.type === 'Receita' ? f.amount : -f.amount} date={f.date ?? f.created_at} player={f.player?.name} />
              ))
          }
        ...
        <div style={{ padding: 24, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {recentPlayers.length === 0
            ? <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', textAlign: 'center', padding: '32px 0', fontWeight: 700, textTransform: 'uppercase' }}>Sem atletas</p>
            : <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {recentPlayers.map(p => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer' }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 11, fontWeight: 900, color: '#fff', textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
                      <p style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', fontWeight: 700 }}>{p.posicao_principal || p.positions?.[0] || '—'} · {p.status}</p>
                    </div>
                    <div style={{ padding: '2px 8px', background: `${gold}12`, border: `1px solid ${gold}25`, fontSize: 9, fontWeight: 900, color: gold }}>
