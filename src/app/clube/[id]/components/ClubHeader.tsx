...
import {
  ...
  faUsers, faWallet, faCalendarDays, faTrophy,
...
export const ClubHeader: React.FC<Props> = ({
  ...
  const balanceColor = summary.balance >= 0 ? '#22c55e' : '#ef4444';

  return (
    <div>
      {/* Banner topo */}
      <div style={{
        background: 'linear-gradient(135deg,rgba(0,0,0,0.97) 0%,rgba(0,12,30,0.97) 50%,rgba(0,0,0,0.97) 100%)',
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 8, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.35em', color: blue }}>
                CLUBE PRO
              </span>
              {isOwner && (
                <span style={{ fontSize: 7, fontWeight: 900, padding: '2px 6px', background: `${gold}18`, border: `1px solid ${gold}33`, color: gold, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                  OWNER
                </span>
              )}
            </div>
            ...
            {group.description && (
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>{group.description}</p>
            )}
            {group.founded_year && (
          ...
          <div style={{ display: 'flex', gap: 16, flexShrink: 0 }}>
            {[
              ...
              <div key={label} style={{ textAlign: 'center' }}>
                <FontAwesomeIcon icon={icon} style={{ color, fontSize: 14, marginBottom: 4 }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
            <button
              onClick={() => onNavigate(`/dashboard/${group.slug}/matches`)}
              style={{
            ...
            >
              <FontAwesomeIcon icon={faPlus} /> Nova Partida
            </button>
            <button
              onClick={onCopyLink}
              ...
              style={{
                display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px',
                background: 'transparent', color: copied ? '#22c55e' : blue,
                border: `1px solid ${copied ? '#22c55e' : blue}33`,
                fontWeight: 900, fontSize: 10, textTransform: 'uppercase',
                letterSpacing: '0.2em', cursor: 'pointer', transition: 'all .2s',
              }}
            >
